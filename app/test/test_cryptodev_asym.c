...
static int
test_cryptodev_asym_ver(union test_case_structure *test_case_data,
						struct rte_crypto_op *result_op)
{
	...
	switch (test_case_data->modex.xform_type) {
		case RTE_CRYPTO_ASYM_XFORM_MODEX:
			data_expected = test_case_data->modex.reminder.data;
			data_received = result_op->asym->modex.base.data;
			data_size = result_op->asym->modex.base.length;
			break;
		case RTE_CRYPTO_ASYM_XFORM_MODINV:
			data_expected = test_case_data->modinv.inverse.data;
			data_received = result_op->asym->modinv.base.data;
			data_size = result_op->asym->modinv.base.length;
			break;
...
test_cryptodev_asym_op(struct crypto_testsuite_params *ts_params,
					union test_case_structure *test_case_data,
					char *test_msg)
{
	...
	asym_op = op->asym;
	xform_tc.next = NULL;
	xform_tc.xform_type = test_case_data->modex.xform_type;
	...
	switch (xform_tc.xform_type) {
		case RTE_CRYPTO_ASYM_XFORM_MODEX:
			xform_tc.modex.modulus.data = test_case_data->modex.modulus.data;
			xform_tc.modex.modulus.length = test_case_data->modex.modulus.len;
			...
			asym_op->modex.base.length = test_case_data->modex.base.len;
		...
		case RTE_CRYPTO_ASYM_XFORM_MODINV:
			...
			asym_op->modinv.base.length = test_case_data->modinv.base.len;
			if (rte_cryptodev_asym_xform_capability_check_modlen(capability,
	...
	if (test_cryptodev_asym_ver(test_case_data, result_op) != TEST_SUCCESS) {
		snprintf(test_msg, ASYM_TEST_MSG_LEN,
			"line %u FAILED: %s",

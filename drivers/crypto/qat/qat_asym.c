...
error:
	...
	cookie->error |= QAT_ASYM_ERROR_DEVIDE_BY_ZERO;
		...
		void *op_cookie)
{
	...
	*op = rx_op;
	...
	if (unlikely(ICP_QAT_FW_PKE_RESP_PKE_STAT_GET(
			resp_msg->pke_resp_hdr.resp_status.pke_resp_flags)))
		rx_op->status = RTE_CRYPTO_OP_STATUS_ERROR;
	if (unlikely(resp_msg->pke_resp_hdr.resp_status.comn_err_code))
		rx_op->status = RTE_CRYPTO_OP_STATUS_ERROR;
	/* In general we need to see running other algos if we need other
	 * error codes
	 */
